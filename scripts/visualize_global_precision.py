...
def get_country_level_stats(iso, level):
    ...
    stats = {}
    stats['lineres_mean'] = np.mean(lineres) if lineres else None
    stats['lineres_std'] = np.std(lineres) if lineres else None
    return stats
...
for level in range(0, 4+1):
    ...
    save_map(geoj, 'lineres_std{}'.format(level),
             'ADM{} Line Prec.\nVariation (m)'.format(level),
             'figures/lineres_std{}.png'.format(level),
             reverse_colors=True)
